...
class RankingService {
    async getTopRatedWorkers(lat, lng, userId = null, category = null) {
        ...
        for (const worker of candidates) {
            const scorePayload = await this.computeWorkerScore(worker, lat, lng, userId, category);
            scoredWorkers.push({
                id: worker.id,
    ...
    async computeWorkerScore(worker, customerLat, customerLng, userId = null, category = null) {
    ...
    async calculateDLRankingScore(worker, distanceKm, jobPrice, jobId = null) {
        ...
        const workerFeatures = await featureStore.getWorkerFeatures(worker.id);
        ...
        const features = {
            ...
            acceptance_rate: workerFeatures.acceptance_rate || 1.0,
